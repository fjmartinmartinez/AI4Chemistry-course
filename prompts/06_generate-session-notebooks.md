...
- All workshops run as Jupyter notebooks executed in the course's own `ai4chem` environment (`env/environment.yml`) — no Google Colab path. Weeks 08-09: a heavier dependency (`transformers`, training-from-scratch) still needs a laptop-CPU-friendly budget — use a reduced-size working set (`N_MAX`) or reduced-epoch budget (`N_EPOCHS`) as needed, but no Colab-detection branching.
